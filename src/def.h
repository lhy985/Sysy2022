...
enum node_kind  {COMP_UNIT,FUNC_DEF,VAR_DEF_LIST,INIT_VAL_LIST,VAR_DECL,INT_TYPE,FLOAT_TYPE,VOID_TYPE,VAR_DEF,INIT_VAL,PARAM_LIST
                ...
                ,RETURN_STMT,FOR_STMT,FOR_ARGS,LVAL,ASSIGN_EXP,AND_EXP,OR_EXP,RELOP_EXP,ADD_EXP,MINUS_EXP,MUL_EXP,DIV_EXP
                ,MOD_EXP,SELF_ADD_EXP,SELF_MINUS_EXP,UMINUS_EXP,NOT_EXP,FUNC_CALL,INT_CONST,FLOAT_CONST,ARGS};
...
typedef struct symbol_scope_begin {  /*当前作用域的符号在符号表的起始位置序号,这是一个栈结构，/每到达一个复合语句，将符号表的index值进栈，离开复合语句时，取其退栈值修改符号表的index值，完成删除该复合语句中的所有变量和临时变量*/
    int TX[30];
    int top;
    } symbol_scope_TX;
